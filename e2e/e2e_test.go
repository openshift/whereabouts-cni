		...
		Context("stateful set tests", func() {
			...
			Context("reclaim previously allocated IP", func() {
				...
				BeforeEach(func() {
					...
					ipPool, err := clientInfo.WbClient.WhereaboutsV1alpha1().IPPools(ipPoolNamespace).Get(context.Background(), wbstorage.NormalizeRange(rangeWithTwoIPs), metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred())
					...
					for _, ip := range secondaryIPs {
						overlapping, err := clientInfo.WbClient.WhereaboutsV1alpha1().OverlappingRangeIPReservations(ipPoolNamespace).Get(context.Background(), ip, metav1.GetOptions{})
						Expect(err).NotTo(HaveOccurred())
				...
				It("can reclaim the previously allocated IPs", func() {
					...
					for i, ip := range secondaryIPs {
						...
						Expect(err).NotTo(HaveOccurred())
						offset := allocate.IPGetOffset(net.ParseIP(ip), firstIP)
						Expect(err).NotTo(HaveOccurred())
...
func verifyNoAllocationsForPodRef(clientInfo *wbtestclient.ClientInfo, ipv4TestRange, testNamespace, podName string, secondaryIfaceIPs []string) {
	Eventually(func() bool {
		ipPool, err := clientInfo.WbClient.WhereaboutsV1alpha1().IPPools(ipPoolNamespace).Get(context.Background(), wbstorage.NormalizeRange(ipv4TestRange), metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
	...
	for _, ip := range secondaryIfaceIPs {
		Eventually(func() bool {
			_, err := clientInfo.WbClient.WhereaboutsV1alpha1().OverlappingRangeIPReservations(ipPoolNamespace).Get(context.Background(), ip, metav1.GetOptions{})
